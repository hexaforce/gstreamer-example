...
void soup_websocket_handler(G_GNUC_UNUSED SoupServer *server, SoupWebsocketConnection *connection, G_GNUC_UNUSED const char *path, G_GNUC_UNUSED SoupClientContext *client_context, gpointer user_data) {
  ...
  //     "videoconvert ! "
  //     "queue max-size-buffers=1 ! "
  //     "av1enc cpu-used=8 keyframe-max-dist=15 ! "
  //     "video/x-av1,stream-format=obu-stream,alignment=tu,parsed=true ! "
  //     "queue max-size-time=100000000 ! "
  //     "rtpav1pay name=payloader ! "
  //     "application/x-rtp,media=video,encoding-name=AV1,payload=" RTP_PAYLOAD_TYPE " ! "
  //     "webrtcbin. "
  //     "autoaudiosrc ! "
  //     "queue max-size-buffers=10 max-size-bytes=0 max-size-time=100000000 leaky=downstream ! "
  //     "audioconvert ! audioresample ! "
  ...
  //     "videoconvert ! "
  //     "queue max-size-buffers=1 ! "
  //     "rav1enc speed-preset=10 quantizer=50 keyframe-interval=15 ! "
  //     "video/x-av1 ! "
  //     "queue max-size-time=100000000 ! "
  //     "rtpav1pay name=payloader ! "
  //     "application/x-rtp,media=video,encoding-name=AV1,payload=" RTP_PAYLOAD_TYPE " ! "
  ...
  //     "videoconvert ! "
  //     "queue max-size-buffers=1 ! "
  //     "svtav1enc preset=8 tune=0 keyint=15 ! "
  //     "video/x-av1 ! "
  //     "queue max-size-time=100000000 ! "
  //     "rtpav1pay name=payloader ! "
  //     "application/x-rtp,media=video,encoding-name=AV1,payload=" RTP_PAYLOAD_TYPE " ! "
