...
static GOptionEntry entries[] = {
    ...
    {"our-id", 0, 0, G_OPTION_ARG_STRING, &our_id, "String ID of the session that peer can connect to us", "ID"},
...
static void data_channel_on_error(GObject *dc, gpointer user_data) { cleanup_and_quit_loop("Data channel error", 0); }
...
static void data_channel_on_close(GObject *dc, gpointer user_data) { cleanup_and_quit_loop("Data channel closed", 0); }

static void data_channel_on_message_string(GObject *dc, gchar *str, gpointer user_data) { gst_print("Received data channel message: %s\n", str); }
...
static gboolean start_pipeline(gboolean create_offer)
{
  ...
  pipe1 = gst_parse_launch("webrtcbin bundle-policy=max-bundle name=sendrecv " STUN_SERVER "videotestsrc is-live=true pattern=ball ! videoconvert ! queue ! "
...
static void connect_to_websocket_server_async(void)
{
  ...
  g_object_set(session, "use-system-ca-file", TRUE, NULL);
  g_object_set(session, "https-aliases", https_aliases, NULL);

  logger = soup_logger_new(SOUP_LOGGER_LOG_BODY);
  soup_session_add_feature(session, SOUP_SESSION_FEATURE(logger));
  g_object_unref(logger);

  message = soup_message_new("GET", server_url);

  gst_print("Connecting to server...\n");

  /* Once connected, we will register */
  soup_session_websocket_connect_async(session, message, NULL, NULL, 0, NULL, (GAsyncReadyCallback)on_server_connected, message);
